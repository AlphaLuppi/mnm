...
> **Deadline critique** : Demo votre organisation — Juin 2026
...
- Matrice Parallelisme MnM founder / Cofondateur
...
- Repartition MnM founder vs Cofondateur
...
> **Split Cofondateurs** : MnM founder (MnM co-founder dev) = Noyaux B + C | Cofondateur = Noyaux A + D | Noyau E = Partage
...
**Total** : ~13 semaines (Sprint 0 + 6 sprints de 2 semaines) = **Demo votre organisation juin 2026**.

### 1.2 Principes de planification

1. **Parallelisme maximal** : MnM founder et Cofondateur travaillent sur des noyaux distincts sauf Noyau E (partage explicite)
...
**Assignation** : Partage MnM founder + Cofondateur
...
**Sprint** : Sprint 1
**Assignation principale** : MnM founder (frontend) + Cofondateur (backend)
...
**Assignation principale** : Cofondateur (backend critique) + MnM founder (UI)
...
**Assignation** : **MnM founder** (noyau principal)
...
**Sprint** : Sprint 4
**Assignation** : Partage MnM founder (UI) + Cofondateur (backend)
...
**Sprint** : Sprint 4
**Assignation** : **Cofondateur** (backend) + MnM founder (UI monitoring)
**Effort total** : ~4-5 semaines
...
| Sprint | MnM founder (MnM co-founder dev) | Cofondateur | Livrable |
|--------|-------------|-------------|----------|
...
| **M7 — Demo votre organisation** | Fin S6 (semaine 13) | Import Jira, dashboards, SSO, stabilisation |
...
## 14. Matrice Parallelisme MnM founder / Cofondateur

### 14.1 Repartition par noyau

| Noyau | MnM founder (MnM co-founder dev) | Cofondateur | Points de synchronisation |
|-------|-------------|-------------|--------------------------|
...
1. **Daily standup 15 min** : MnM founder et Cofondateur synchronisent chaque matin sur les blocages et les interfaces
2. **Spec API first** : le Cofondateur definit l'API (endpoints, types, responses) AVANT que MnM founder commence la UI
...
|--------|------------|--------|------------|
| API pas prete quand MnM founder commence la UI | Moyenne | Elevee | MnM founder commence par les composants purs (design system, composants sans API) puis branche l'API |
| Conflits de merge sur les types partages | Faible | Moyenne | Un seul mainteneur pour `packages/shared/` (Cofondateur), MnM founder propose des PRs |
...
| **Stories MnM founder** | ~24 stories |
| **Stories Cofondateur** | ~23 stories |
| **Duration totale** | 13 semaines (1 + 6x2) |
| **Demo votre organisation** | Juin 2026 |
...
```
Semaine 0 (Jour 1-3) :
  MnM founder    : INFRA-S01 (PostgreSQL) + INFRA-S03 (Redis)
  Cofond.: INFRA-S02 (Docker Compose) + INFRA-S04 (CI/CD)

Semaine 0-1 (Jour 3-7) :
  MnM founder    : INFRA-S05 (10 nouvelles tables) + INFRA-S06 (5 tables modifiees)
...
- **MnM founder** : Backend + Observabilite (Noyau B + C) — expertise infra, Docker, monitoring
...
| MnM founder (Backend + Observabilite) | Cofondateur (Orchestration + Agents) | Dependances |
|-------------------------------|--------------------------------------|-------------|
...
| INFRA-S05: Schema 10 tables | INFRA-S07: RLS (apres S05) | S07 attend S05 de MnM founder |
| INFRA-S06: Schema modifs | RBAC-S03: Fix hasPermission | S03 attend S06 de MnM founder |
...
**Point de sync fin Sprint 2** : Routes securisees, audit en place, state machine fonctionnelle. MnM founder livre les APIs, le Cofondateur livre le moteur d'orchestration.
...
**Point de sync fin Sprint 4** : Chat temps reel, A2A basique, dashboards, curseur. Feature complete pour demo votre organisation.
...
**Point de sync fin Sprint 5** : Produit B2B vendable. Demo votre organisation possible.

### 6.3 Dependances Bloquantes Inter-Developpeurs

Il y a 4 moments critiques ou un developpeur bloque l'autre :

1. **Sprint 1, Jour 3** : Le Cofondateur ne peut pas commencer RLS avant que MnM founder finisse les schemas
2. **Sprint 1, Jour 5** : Le Cofondateur ne peut pas commencer hasPermission fix avant que MnM founder finisse les modifications de tables
3. **Sprint 3** : Le Chat bidirectionnel (MnM founder) doit etre pret avant que le Chat in-workflow (Sprint 4) puisse etre fait
...
> **Contexte** : Deadline critique — Demo votre organisation en juin 2026


## 2. Scope MVP Strict -- Demo votre organisation Juin 2026

### 2.1 MUST-HAVE : Stories Obligatoires pour la Demo

Ce sont les stories **sans lesquelles la demo votre organisation echoue**. Elles constituent le produit minimum demontrable a un CTO enterprise.
...
| Page Membres avec tableau et filtres | Le CTO votre organisation doit voir qui a acces | CTO | P0 |
...
| Story | Justification Business | Persona | Priorite |
|-------|----------------------|---------|----------|
| 4 roles metier (Admin, Manager, Contributor, Viewer) | Le CTO votre organisation doit assigner des droits differencies | CTO | P0 |
...
Ces stories ameliorent l'experience mais ne bloquent pas la demo votre organisation. Elles peuvent etre reportees a post-demo sans risque commercial.

| Story | Epic | Justification Report | Effort |
|-------|------|---------------------|--------|
| Invitation bulk (CSV) | Multi-User | Demo avec 3-5 users suffit | 1j |
| Selecteur multi-company | Multi-User | votre organisation = 1 company pour la demo | 1j |
...
| Multi-tenant SaaS | Enterprise | Single-tenant pour votre organisation suffit | 3-5j |
...
**Total NICE-TO-HAVE : ~45-55 jours.** A planifier post-demo votre organisation.
...
==========================================  ==========================================
= DEMO votre organisation REUSSIE                         = PRODUIT B2B COMPLET
```

**Avec 2 developpeurs en parallele (MnM founder + Cofondateur), les MUST-HAVE representent 8-10 semaines calendaires** -- ce qui correspond exactement a la timeline mars-juin 2026.

---

## 3. Success Criteria Mesurables par Epic

Les 26 Success Criteria du PRD (section 3) sont ici mappes aux epics concernes, avec les cibles a atteindre pour la demo votre organisation (3 mois).

### 3.1 Multi-User MVP

| SC | Critere | Cible Demo votre organisation | Epic |
|----|---------|---------------|------|
| SC-BIZ-1 | Premier client pilote (votre organisation) | POC signe | Multi-User |
...
| SC | Critere | Cible Demo votre organisation | Epic |
|----|---------|---------------|------|
| SC-BIZ-3 | Roles non-dev actifs | >30% des users votre organisation | RBAC |
| SC-D4 | Taux validation humaine A2A | 100% (tout passe par HITL) | RBAC |
| SC-C4 | Companies avec >=3 niveaux hierarchiques | >50% (votre organisation = 1 company, 3+ niveaux) | Scoping |
...
| SC-A4 | Workflows actifs utilises | 10+ (chez votre organisation) | Orchestrateur |
...
| SC-B3 | Reduction MTTR | -40% vs baseline votre organisation | Observabilite |
...
| SC | Critere | Cible Demo votre organisation | Responsable |
|----|---------|---------------|-------------|
| SC-BIZ-1 | Premier client pilote votre organisation | POC signe | MnM founder + CEO |
...
**Pourquoi le CEO compte pour la demo votre organisation :** Le DPO/CEO de votre organisation est le sponsor du projet. S'il n'a pas de visibilite immmediate, il perdra confiance. Le dashboard executif est son "moment de verite".
...
**Pourquoi le CTO compte pour la demo votre organisation :** C'est le decision-maker technique. S'il detecte un trou de securite ou un manque de rigueur, c'est game over. Les stories RBAC et containerisation repondent directement a ses "questions pieges".
...
**Pourquoi le dev compte pour la demo votre organisation :** Les devs votre organisation sont les early adopters quotidiens. Si le chat est fluide et l'orchestrateur fiable, ils deviennent les evangelistes internes.
...
**Duree estimee :** 1 semaine | **Equipe :** MnM founder
...
**Duree estimee :** 2 semaines | **Equipe :** MnM founder (ou Cofondateur si recrute)
...
**Duree estimee :** 2-3 semaines | **Equipe :** MnM founder
...
**Duree estimee :** 3-4 semaines | **Equipe :** MnM founder + Cofondateur

| Critere Go/No-Go | Seuil | Methode de Verification |
|-------------------|-------|------------------------|
| Audit log immutable (pas d'UPDATE/DELETE) | 100% | Test : tenter un UPDATE sur audit_events -> echec |
| Dashboards CEO et CTO rendus avec donnees reelles | Fonctionnel | Smoke test avec donnees de demo votre organisation |
| SSO SAML/OIDC fonctionnel (si inclus dans le scope demo) | 1 provider | Test : login via SSO votre organisation -> session creee |
| Performance API <500ms P95 | Mesure | Load test basique (50 users simultanes) |

**Condition No-Go :** La Phase 4 est partiellement optionnelle pour la demo votre organisation. SSO et multi-tenant SaaS peuvent etre demontres "en plan" plutot qu'en production si le temps manque.

### 5.5 Vue d'Ensemble Go/No-Go

```
Phase 1 [GO] -----> Phase 2 [GO] -----> Phase 3 [GO] -----> Phase 4 [GO] -----> DEMO votre organisation
  |                   |                   |                   |
...
### 6.1 Risques Classes par Impact sur la Demo votre organisation

| # | Risque | Probabilite | Impact Demo votre organisation | Epics Impactes | Mitigation |
|---|--------|------------|-----------------|----------------|------------|
...
| **R2** | Recrutement cofondateur retarde (>4 semaines) | Moyenne | **ELEVE** -- reduit la capacite de parallelisation, timeline depassee | Tous | Freelance senior pour la Piste B (containerisation). MnM founder prend la Piste A. |
...
CRITICITE ELEVEE (degradent la demo) :
  4. R5 -- Docker @ votre organisation      --> Verifier infra semaine 1
...
## 7. Split Cofondateurs : MnM founder vs Cofondateur

### 7.1 Repartition par Noyau de Valeur

| Cofondateur | Noyaux | Profil Requis | Epics |
|------------|--------|---------------|-------|
| **MnM founder** | B (Observabilite) + C (Onboarding) | Product Engineer -- UI/UX, adoption, import | Multi-User, Observabilite, Onboarding, Dual-Speed (UI) |
...
| Fin S8 | Containerisation + Chat | Go/No-Go Demo votre organisation. Si retard, couper NICE-TO-HAVE restant. |
...
**Risque principal du split :** Le scoping (Phase 3) depend de travaux des DEUX cofondateurs (Multi-User par MnM founder, RBAC backend par le Cofondateur). Si l'un est en retard, le scoping est bloque. **Mitigation :** RBAC backend doit etre termine avant la fin de la semaine 4, sans exception.
...
5. **La demo votre organisation est faisable en 8-10 semaines** avec 2 developpeurs, a condition de respecter STRICTEMENT le perimetre MUST-HAVE. Chaque NICE-TO-HAVE ajoute est une dette sur la timeline.

### 8.2 Criteres de Succes de la Demo votre organisation

Pour que la demo votre organisation soit consideree comme un succes et mene a un POC signe :

- [ ] Le CTO votre organisation peut inviter 3-5 membres avec des roles differents
...
- [ ] Le dev votre organisation dialogue en temps reel avec son agent
...
**Si ces 8 points sont couverts, le POC votre organisation est gagne.**
...
| MU-03 | Invitation bulk (CSV ou liste emails) | S | 2 | 1-2j | MnM founder | Extension de MU-01 : parsing CSV côté client, boucle sur le endpoint d'invitation, feedback par ligne. |
...
| MU-05 | Désactivation signup libre (invitation-only) | S | 1 | 0.5j | MnM founder | Flag `invitationOnly` sur `companies`, check dans auth middleware. Trivial. |
| MU-06 | Sign-out avec invalidation session | S | 1 | 0.5j | MnM founder | Better Auth a déjà le mécanisme. Besoin : bouton UI + appel endpoint + cleanup local. |
| MU-07 | Migration PostgreSQL externe | M | 5 | 3-4j | MnM founder | **Story technique critique**. docker-compose.dev.yml avec PostgreSQL 16, config connexion .env, migration du SQLite embarqué, scripts de seed, documentation setup. Prérequis pour tout le reste. |
...
| Story | Description | Taille | SP | Effort | Assignation | Justification |
|-------|-------------|--------|-----|--------|-------------|---------------|
| RBAC-01 | Fix hasPermission() — lire scope JSONB | M | 3 | 2j | MnM founder | **DETTE TECHNIQUE CRITIQUE (DT1)**. access.ts:45-66 ignore complètement `scope`. Requiert : lecture scope, parsing Zod, validation projectId. Impact : chaque route qui appelle hasPermission() est potentiellement vulnérable. Code existant = 268 lignes dans services/access.ts. Risque modéré car le fix est bien défini dans l'architecture (ADR-002). |
| RBAC-02 | 9 nouvelles permission keys + presets par rôle | S | 2 | 1-2j | MnM founder | constants.ts : ajouter 9 keys (workflows.create/manage, agents.launch, stories.create/edit, audit.view/export, dashboard.view, chat.agent). role-presets.ts : matrice admin/manager/contributor/viewer. Modification simple, bien définie. |
...
| RBAC-04 | Enforcement dans 22 fichiers routes | L | 8 | 5-7j | MnM founder + Cofondateur | **La plus grosse story de l'epic.** Audit des 22 fichiers dans server/src/routes/. 3 fichiers critiques sans aucun check (approvals.ts, assets.ts, secrets.ts). Besoin : middleware `requirePermission(key, scopeExtractor?)` appliqué systématiquement. Travail mécanique mais volumineux et critique pour la sécurité. Splitting recommandé : MnM founder fait les routes critiques (approvals, assets, secrets, agents, workflows), Cofondateur fait le reste. |
...
| Story | Description | Taille | SP | Effort | Assignation | Justification |
|-------|-------------|--------|-----|--------|-------------|---------------|
| PROJ-01 | Table project_memberships + migration | S | 2 | 1j | MnM founder | Nouvelle table Drizzle : id, userId, projectId, companyId, role, grantedBy, createdAt. Relations → users, projects, companies. Migration straightforward. |
| PROJ-02 | Service project-memberships.ts | M | 3 | 2-3j | MnM founder | CRUD complet + validation scope JSONB dans hasPermission(). Intégration avec le fix RBAC-01. Filtrage des projets visibles par user. |
| PROJ-03 | Filtrage agents/issues par scope projet | M | 5 | 3-5j | MnM founder | Modification de agents.ts (22782 lignes service, 52248 lignes routes) et issues.ts (50807 lignes service). Ajout de WHERE projectId IN (user_project_ids) sur toutes les queries. **Attention** : ces fichiers sont massifs, risque de régression élevé. Tests d'intégration obligatoires. |
...
| ORCH-04 | API routes orchestrateur | M | 3 | 2-3j | MnM founder | POST /workflows/:id/enforce, GET /drift/alerts, POST /drift/alerts/:id/resolve. Routes Express standard avec auth + RBAC. |
...
**Total Epic : 13-16 SP | ~1.5-2 semaines (principalement MnM founder)**

| Story | Description | Taille | SP | Effort | Assignation | Justification |
|-------|-------------|--------|-----|--------|-------------|---------------|
| DRIFT-01 | Fix drift en mémoire → persistance DB | M | 3 | 2-3j | MnM founder | **DETTE TECHNIQUE (DT2)**. drift.ts (405 lignes) utilise `reportCache = new Map()` — perdu au restart. Besoin : tables drift_reports + drift_items dans PostgreSQL. Migration des données in-memory vers DB. MnM founder connaît bien ce code. |
| DRIFT-02 | Drift monitor service | M | 5 | 3-5j | MnM founder | Nouveau service drift-monitor.ts : comparaison attendu (workflow template) vs observé (heartbeat events). Alertes temps réel via WebSocket (live-events.ts existant). Intégration avec la state machine ORCH-01. |
...
| Story | Description | Taille | SP | Effort | Assignation | Justification |
|-------|-------------|--------|-----|--------|-------------|---------------|
| OBS-01 | Table audit_events + migration | M | 5 | 3-4j | MnM founder | **Table critique.** Partitionnée par mois (PARTITION BY RANGE createdAt). TRIGGER deny UPDATE/DELETE. Colonnes : companyId, actorId, actorType, action, targetType, targetId, metadata JSONB, ipAddress, prevHash, createdAt. Rôles PostgreSQL séparés (mnm_app vs mnm_audit_admin). Plus complexe qu'une migration standard à cause du partitionnement et des triggers. |
| OBS-02 | Service audit.ts + emission systématique | M | 5 | 3-5j | MnM founder | Service centralisé pour émettre des audit events depuis toutes les routes. Intégration avec les 22 fichiers de routes. Pattern : middleware ou helper `emitAudit(action, target, metadata)`. Volume de travail mécanique mais important. |
| OBS-03 | Résumé LLM actions agent (<5s) | M | 3 | 2-3j | MnM founder | audit-summarizer.ts : appel Haiku pour traduire les logs techniques en langage naturel. Intégration avec heartbeat_run_events. Cache Redis pour éviter les appels LLM redondants. MnM founder connaît bien le heartbeat. |
...
| Story | Description | Taille | SP | Effort | Assignation | Justification |
|-------|-------------|--------|-----|--------|-------------|---------------|
| CHAT-01 | WebSocket bidirectionnel | L | 8 | 5-7j | MnM founder | **DETTE TECHNIQUE (DT3)**. live-events.ts actuel = 40 lignes, unidirectionnel (serveur→client). Besoin : protocole de messages typé bidirectionnel, routing par channelId, authentification WebSocket. Extension majeure du système WebSocket existant. MnM founder connaît ce code. |
| CHAT-02 | Tables chat_channels + chat_messages | S | 2 | 1j | MnM founder | 2 tables Drizzle : chat_channels (id, agentId, heartbeatRunId, companyId, status), chat_messages (id, channelId, senderId, senderType, content, createdAt). Migration standard. |
| CHAT-03 | ChatService + pipe stdin agent | M | 5 | 3-5j | MnM founder | Service chat-service.ts : réception message WebSocket → persistance DB → pipe vers stdin de l'agent containerisé. Reconnexion avec buffer 30s. Rate limiting 10 msg/min. Dépend de la containerisation (Epic 8) pour le pipe stdin. |
...
| CONT-05 | Tables container_profiles + container_instances | S | 2 | 1j | MnM founder | 2 tables Drizzle + migration. container_profiles (name, cpu, memory, disk, timeout, mountAllowlist, companyId), container_instances (profileId, agentId, containerId, status, startedAt, stoppedAt). |
...
**Total Epic : 21-26 SP | ~2.5-3 semaines (Cofondateur, support MnM founder)**
...
**Total Epic : 8-10 SP | ~1 semaine (MnM founder)**

| Story | Description | Taille | SP | Effort | Assignation | Justification |
|-------|-------------|--------|-----|--------|-------------|---------------|
| SSO-01 | Table sso_configurations + config | S | 2 | 1j | MnM founder | Table Drizzle : id, companyId, provider (saml/oidc), config JSONB, enabled. Migration standard. |
| SSO-02 | Better Auth plugins SAML/OIDC | M | 3 | 2-3j | MnM founder | Intégration plugins Better Auth. Config dynamique par company (chaque company peut avoir son propre IdP). Flux : user → login page → redirect IdP → callback → création/mapping user → session. |
...
| Story | Description | Taille | SP | Effort | Assignation | Justification |
|-------|-------------|--------|-----|--------|-------------|---------------|
| DASH-01 | API dashboards agrégés | M | 5 | 3-4j | MnM founder | Endpoints GET /dashboards/:type (executive, technical, operational). Queries agrégées avec k-anonymity (k=5). Vues matérialisées PostgreSQL avec GROUP BY obligatoire. Interdiction architecturale du drill-down individuel (Vérité #20). |
...
| DASH-03 | Dashboard temps réel via WebSocket | S | 2 | 1-2j | MnM founder | Push updates dashboard via WebSocket existant. Rafraîchissement automatique des métriques toutes les 30s. |
...
|---|-------|--------|-----|--------|-------------|---------------|--------|
| TECH-01 | Docker Compose dev/test/prod | M | 5 | 3-4j | MnM founder | docker-compose.dev.yml (PostgreSQL 16, Redis 7, server, ui), docker-compose.test.yml (PostgreSQL de test), Dockerfile server + ui. **Prérequis pour toute containerisation.** | Epic 8 |
| TECH-02 | CI/CD Pipeline GitHub Actions | L | 8 | 5-7j | MnM founder | Pipeline complet : QG-0 (lint+TS) → QG-1 (unit) → QG-2 (integration) → QG-3 (security) → QG-4 (perf) → QG-5 (E2E) → QG-6 (review). Caching pnpm, Docker layers, Cypress binary. ~15-22 min. | Déploiement |
| TECH-03 | Infrastructure de test (factories + seed) | M | 5 | 3-4j | MnM founder | Factories TypeScript : createTestUser(), createTestCompany(), createTestAgent(), createTestWorkflow(). Seed E2E pour Cypress. Embedded-postgres pour tests d'intégration. Pattern mock : mocker LLM providers, JAMAIS la DB. | Tous les tests |
| TECH-04 | Redis setup + session/cache | M | 3 | 2-3j | MnM founder | Redis pour : sessions (au lieu de DB), query cache, WebSocket pub/sub (multi-instance), rate limiting. docker-compose + config Node. | Performance, scaling |
| TECH-05 | RLS PostgreSQL (14 tables) | L | 8 | 5-7j | MnM founder | **Story de sécurité critique.** CREATE POLICY sur 14 tables (toutes sauf user, session, account, verification, instance_user_roles, migrations, assets, inbox_dismissals). SET LOCAL app.current_company_id au début de chaque requête. Tests d'isolation cross-company. | Multi-tenant |
| TECH-06 | Migration DB : 10 nouvelles tables | M | 5 | 3-4j | MnM founder | Création des 10 tables listées dans le domain model (Section 5.3 PRD). Migration Drizzle avec rollback scripts. Seed de données de test. | Toutes les epics |
| TECH-07 | Migration DB : colonnes ajoutées sur 5 tables | S | 2 | 1j | MnM founder | companies (+tier, ssoEnabled, maxUsers, parentCompanyId), company_memberships (+businessRole), agents (+containerProfileId, isolationMode), principal_permission_grants (+9 keys), activity_log (+ipAddress, userAgent, severity). | Epic 2, 3 |
| TECH-08 | Refactoring heartbeat.ts (2396 → modules) | M | 5 | 3-5j | MnM founder | **DETTE TECHNIQUE (DT7)**. Splitting en modules : heartbeat-core.ts, heartbeat-events.ts, heartbeat-compaction.ts, heartbeat-metrics.ts. Nécessaire avant d'intégrer le CompactionWatcher (Epic 9). | Epic 9 |
...
**Chemin A (Product/RBAC)** — MnM founder principalement :
```
...
## 6. Répartition MnM founder vs Cofondateur

### 6.1 MnM founder (Cofondateur existant — Backend + Observabilité)

MnM founder connaît la codebase, donc il prend :
...
**Charge MnM founder : ~115-140 SP | ~12-14 semaines full-time**
...
|---------|-------|-------------|
| S1 | Setup Docker + PostgreSQL + premiers tests | MnM founder + Cofondateur |
| S2 | Review RBAC-01 (fix sécurité scope) | MnM founder → Cofondateur review |
| S4 | Intégration state machine + heartbeat | Cofondateur → MnM founder review |
| S6 | Review credential proxy + container security | Cofondateur → MnM founder + externe |
...
**Jalon votre organisation (juin 2026)** : Après Sprint 5 (~S12 = début juin), le MVP est **vendable** avec : multi-user, RBAC, scoping, orchestrateur v1, drift basique, containerisation P1. C'est le moment de la démo votre organisation.
...
|-------|-----------------|--------|
| ORCH-05 (UI drag-and-drop workflow) | P1, éditeur YAML suffit pour le MVP votre organisation | Faible — CTO votre organisation peut configurer en YAML |
...
| SSO-01/02/03 | P1 Enterprise, pas nécessaire pour le POC votre organisation | Nul pour le POC — votre organisation utilise email/password |
| OBS-03 (résumé LLM) | P1, les logs techniques bruts suffisent pour le MVP | Moyen — expérience dégradée mais fonctionnelle |

**En reportant ces stories (~25-30 SP), le MVP votre organisation tombe à ~220-275 SP soit 8-10 sprints (16-20 semaines).**
...
**Pour respecter la deadline votre organisation (juin 2026)**, il faut :
...
- Accepter un MVP "rugueux" pour le POC votre organisation et polir en Sprint 6-10
...
*47 stories, 12+ epics, 7 sprints, 197 SP planifies (MVP votre organisation), timeline 13 semaines, ~525 tests prevus, 13 criteres DoD, 31 ACs Given/When/Then, 42 edge cases.*
