...
L'information se dégrade à chaque passage de relais entre rôles (PPT → Epic → Story → Code → Tests). Les contrats inter-rôles sont aspirationnels, jamais appliqués. Les agents IA sont puissants mais dérivent sans contrôle — comme l'a constaté le CTO de EnterpriseCustomer au hackathon de mars 2026.
...
**Contenu** : 16 epics, ~69 stories, 252-307 SP total, 100% des 52 FRs couverts, 8 ADRs mappés, 42 critères DoD (4 niveaux), 31 ACs Given/When/Then, 42 edge cases, ~525 tests estimés, timeline 13 semaines (MVP EnterpriseCustomer) à 20-22 semaines (complet).

**Décision clé** : Alerte timeline (Amelia) — le PRD annonce 8-10 semaines, l'estimation réaliste est 20-22 semaines. Pour respecter EnterpriseCustomer juin 2026 : focus strict MUST-HAVE, reporter SSO/dashboards avancés/résumé LLM au post-MVP.
...
[INFRA][MU+RBAC ][RBAC+ORCH][ORCH+OBS][CONT+CHAT][A2A+DUAL ][ENT+DEMO]
  ↑                              ↑                   ↑            ↑
  PG+Docker                  Démo interne       MVP rugueux   🎯 DÉMO EnterpriseCustomer
...
| Juin 2026 | Sprint 6 — **🎯 Démo EnterpriseCustomer** |
...
5. **Vérifier l'infra EnterpriseCustomer** — Docker est-il disponible chez EnterpriseCustomer ? Si non, préparer le Plan B (processus isolés).
...
8. **Contacter EnterpriseCustomer** — Valider l'intérêt pour le POC, aligner les attentes sur le périmètre démo.
