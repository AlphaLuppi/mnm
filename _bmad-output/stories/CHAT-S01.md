...
Le chat bidirectionnel est le "moment emotionnel de la demo" (epics-b2b.md, impact 7/10). Aujourd'hui, `live-events-ws.ts` est strictement unidirectionnel : le serveur pousse des LiveEvents vers les clients, mais les clients ne peuvent rien envoyer en retour. Pour la demo votre organisation, un dev doit pouvoir taper "Utilise le pattern Repository" dans un chat et voir l'agent s'adapter en temps reel.
