...
**Participants :** MnM founder, Claude (Opus 4.7 1M)
...
6. Puis MnM founder a rappelé que **les gates doivent bien être EXÉCUTÉES côté serveur** — ce qui rouvre la question du format (si les gates sont du code custom TS, on a besoin de TS ; si elles sont déclaratives, JSON suffit).
...
11. **Token MnM central + commit author = user réel.** Token = simple (MnM écrit au nom d'un bot), author du commit = l'user qui a déclenché la modif (MnM contributor, Paul, etc.). Traçabilité via git log.
...
17. **Gates = évaluées côté serveur** sur les artifacts reportés par le client. ← MnM founder a explicitement rappelé ce point en fin de session. Reste à trancher : gates = code custom TS sandboxé OU gates = identifiers déclaratifs.
...
- Attaquer direct le point 1 (nature des gates) : MnM founder doit trancher A ou B.
