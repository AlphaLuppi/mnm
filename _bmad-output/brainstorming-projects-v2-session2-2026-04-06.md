...
- **Ne pas créer une usine à gaz spécifique à EnterpriseCustomer** — Tom est biaisé par ce modèle
...
Pas d'entités EnterpriseCustomer-spécifiques. MnM doit marcher pour une startup de 3 (AlphaLuppi) comme pour une boîte réglementée de 500. La flexibilité vient des Blocks + agents configurables.
...
Un projet = un produit complet. Exemple <external-stakeholder>@EnterpriseCustomer : app mobile + app web Angular + vieille app internal-backend. Un produit = ensemble de features réparties sur N codebases. Chaque codebase = un workspace avec son MCP GitNexus.

### Insight J : Prototype deployment = MVP scope (anti shadow IT)
Le déploiement de prototypes par les PM est dans le scope MVP. Raison : éviter la shadow IT — PMs qui vibecode sur Vercel/Netlify et font fuiter des infos. MnM = outil officiel pour prototyper ET déployer. L'orchestrateur + features MnM répondent aussi à l'élimination de la shadow IA. Équipe infra/SRE EnterpriseCustomer aide sur cette partie.
