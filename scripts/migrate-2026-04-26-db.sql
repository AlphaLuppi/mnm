...
-- (mnm-workflows-demo) or the new (mnm-demo) repo path. We do this in a
...
BEGIN
  ...
  WHERE config_json->>'kind' = 'gitlab'
    AND (
      config_json->>'projectId' LIKE '%mnm-workflows-demo%'
      OR config_json->>'projectId' LIKE '%mnm-demo%'
    );

  IF match_count = 0 THEN
    RAISE EXCEPTION 'No gitlab config_layer_item found pointing at mnm-workflows-demo or mnm-demo. Aborting M2.';
