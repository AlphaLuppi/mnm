...
MNM_HOST_PORT="${MNM_HOST_PORT:-3100}"
MNM_HOST_FROM_CONTAINER="${MNM_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.mnm-openclaw.override.yml"
...
detect_mnm_base_url() {
  ...
  for candidate in "$MNM_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${MNM_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${MNM_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
mnm_base_url="$(detect_mnm_base_url || true)"
...
MnM URL for OpenClaw container:
EOF
  if [[ -n "$mnm_base_url" ]]; then
    cat <<EOF
  $mnm_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${MNM_HOST_PORT}
  (Do not use http://127.0.0.1:${MNM_HOST_PORT} inside the container.)
  If MnM rejects the host, run on host machine:
    bun run mnm allowed-hostname host.docker.internal
  Then restart MnM and re-run this script.
EOF
  fi
  cat <<EOF
