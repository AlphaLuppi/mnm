...
MNM_API_URL="${MNM_API_URL:-http://127.0.0.1:3100}"
API_BASE="${MNM_API_URL%/}/api"

COMPANY_SELECTOR="${COMPANY_SELECTOR:-DEMO}"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-mnm-smoke}"
...
MNM_API_URL_FOR_OPENCLAW="${MNM_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${MNM_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${MNM_AUTH_HEADER}" )
fi
if [[ -n "${MNM_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${MNM_COOKIE}" )
  MNM_BROWSER_ORIGIN="${MNM_BROWSER_ORIGIN:-${MNM_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${MNM_BROWSER_ORIGIN}" -H "Referer: ${MNM_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set MNM_COOKIE or MNM_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg mnmApiUrl "$MNM_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/mnm"
  local claimed_file="${workspace_dir}/mnm-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/mnm-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/mnm-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/mnm-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/mnm"
    assert_status "200"
    {
      echo "MNM_API_URL=${MNM_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set MNM_API_KEY=${AGENT_API_KEY} in your run context before running MnM heartbeat steps."
...
main() {
  ...
  wait_http_ready "${MNM_API_URL%/}/api/health" 15 || fail "MnM API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "mnm health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
