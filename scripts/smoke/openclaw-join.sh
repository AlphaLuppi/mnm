...
COMPANY_ID="${COMPANY_ID:-${MNM_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-mnm-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-mnm-openclaw-smoke}"
...
Provide one of:
  MNM_AUTH_HEADER=\"Bearer <board-token>\"
  MNM_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "MnM OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.mnm.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
