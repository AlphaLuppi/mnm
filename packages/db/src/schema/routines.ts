import { pgTable, uuid, text, timestamp, jsonb, index, integer, uniqueIndex, boolean } from "drizzle-orm/pg-core";
...
export const routines = pgTable(
  "routines",
  {
    ...
    variables: jsonb("variables").$type<RoutineVariable[]>().default([]),
...
export const routineTriggers = pgTable(
  ...
  (table) => ({
    ...
    nextRunAtIdx: index("routine_triggers_next_run_at_idx").on(table.nextRunAt),
    publicIdIdx: uniqueIndex("routine_triggers_public_id_idx").on(table.publicId),
  }),
);
...
export const routineRuns = pgTable(
  ...
  (table) => ({
    companyRoutineIdx: index("routine_runs_company_routine_idx").on(table.companyId, table.routineId),
