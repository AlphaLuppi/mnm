...
describe("CompiledCache", () => {
  it("stores and retrieves by (gitSha, path)", () => {
    ...
    cache.set("sha1", "hooks/a.hook.ts", "var js1 = 1;");
    expect(cache.get("sha1", "hooks/a.hook.ts")).toBe("var js1 = 1;");
  });

  it("returns undefined on miss", () => {
    const cache = new CompiledCache();
    expect(cache.get("sha1", "hooks/a.hook.ts")).toBeUndefined();
  });

  it("treats different shas as different keys", () => {
    const cache = new CompiledCache();
    cache.set("sha1", "hooks/a.hook.ts", "v1");
    cache.set("sha2", "hooks/a.hook.ts", "v2");
    expect(cache.get("sha1", "hooks/a.hook.ts")).toBe("v1");
    expect(cache.get("sha2", "hooks/a.hook.ts")).toBe("v2");
  });

  it("treats different paths as different keys", () => {
    const cache = new CompiledCache();
    cache.set("sha1", "hooks/a.hook.ts", "v-a");
    cache.set("sha1", "hooks/b.hook.ts", "v-b");
    expect(cache.get("sha1", "hooks/a.hook.ts")).toBe("v-a");
    expect(cache.get("sha1", "hooks/b.hook.ts")).toBe("v-b");
  });
