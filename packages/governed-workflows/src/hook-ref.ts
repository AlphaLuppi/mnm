 ...
 *
 * Validation rules (T2.6 — P4-F: aligned on `{ref, config}` to match
 * REST DTO + catalog vocabulary, was `{name, with}`):
 *  - `ref` MUST match `^(canonical|shared|local):[a-z0-9-]+$`. The
 ...
 *    time.
 *  - `config` is the per-step config object passed to the hook as
