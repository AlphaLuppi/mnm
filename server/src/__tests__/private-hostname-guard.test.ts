...
describe("privateHostnameGuard", () => {
  it("allows requests when disabled", async () => {
    ...
    const res = await request(app).get("/api/health").set("Host", "dev-host-1:3100");
  ...
  it("allows explicitly configured hostnames", async () => {
    const app = createApp({ enabled: true, allowedHostnames: ["dev-host-1"] });
    const res = await request(app).get("/api/health").set("Host", "dev-host-1:3100");
    expect(res.status).toBe(200);
  });
  ...
  it("blocks unknown hostnames with remediation command", async () => {
    ...
    expect(res.body?.error).toContain("please run bun run mnm allowed-hostname dev-host-1");
  });

  it("blocks unknown hostnames on page routes with plain-text remediation command", async () => {
    const app = createApp({ enabled: true, allowedHostnames: ["some-other-host"] });
    const res = await request(app).get("/dashboard").set("Host", "dev-host-1:3100");
    expect(res.status).toBe(403);
    expect(res.text).toContain("please run bun run mnm allowed-hostname dev-host-1");
  });
});
