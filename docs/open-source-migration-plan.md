...
- `_bmad-output/` (75 fichiers de brainstorming, vision, planning) — tous anonymisés (client, produit client, MnM founder/MnM co-founder dev/MnM CEO, paths perso).
...
- `<personal-username>` → `your-username` ; `<contributor-name>` → `MnM contributor`
...
- Prénoms équipe : `Gabriel`/`MnM co-founder dev` → `MnM co-founder dev` ; `MnM CEO`/`MnM CEO` → `MnM CEO` ; `Thomas`/`MnM founder Andrieu` → `MnM founder`/`MnM contributor`.
- Plugins Claude Code customer-spécifiques : `<client>-dev-angular` → `team-dev-angular`, `<client>-dev-java` → `team-dev-java`, `<client>-dev-workflows` → `team-dev-workflows`, etc.
- Produit client : nom produit → `<customer-product>` ; sous-produits → `enterprise-product-{web,mobile}`.
...
| `_bmad-output/brainstorming/brainstorming-enterprise-product-refonte-2026-04-17.md` | Brainstorm client refonte | À déplacer hors repo public |
| `_bmad-output/brainstorming/dm-hackathon.md` | DMs réels (PO, QA, CEO, DSI, DPO, CTO, Architecte) | À supprimer ou déplacer hors repo public |
| `_bmad-output/brainstorming/interview-questions-hackathon.md` | Questions interview événement interne | À supprimer ou déplacer hors repo public |
...
- **Sed pass 2 (résiduels prénoms)** : `Tom`/`Gab`/`Gabri`/`Nikou`/`Niko` standalone (manqués par la passe 1) + frontmatter YAML `user_name: Tom`.
- **Sed pass 3 (jargon sectoriel)** : `IDEL` → professionnel de santé ; `HAS` → autorité de régulation sectorielle ; `NGAP` → nomenclature sectorielle ; `cabinet` → local ; `ordonnance` → document généré ; `médecin` → professionnel ; `prescription` → génération de document ; `mutuelles` → tiers payeurs ; stack client (`Struts`/`app-revolution`/`emotion-back`/`Capacitor`/`Ionic`/`Detox`) → tournures génériques (`legacy-backend`, `hybrid-mobile-stack`, etc.) ; `enterprise-product` → `legacy-app`.
