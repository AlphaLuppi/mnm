# PRD — Refonte page d'authentification AgatheYou

**Feature** : Refonte page d'authentification AgatheYou
...
La page d'authentification actuelle d'AgatheYou (web + mobile) souffre de plusieurs limitations :

- **UX vieillissante** : design antérieur au refresh du design system CBA, incohérent avec le reste de l'app.
...
- **Pas d'analytics** : on ne sait pas combien d'IDEL abandonnent la connexion, ni à quelle étape.

## Solution

Refonte complète de la page d'authentification sur les deux plateformes (web Angular + mobile Ionic) avec :

1. **Nouvelle UI** alignée sur le design system CBA (UI Kit `angular-libs`), avec support dark mode.
2. **Login unifié email/mot de passe** + SSO OAuth (Google, Microsoft) pour les cabinets en mode multi-IDEL.
...
- Page de login web (agathe-you-web) : `/auth/login`.
- Page de login mobile (app-revolution) : écran initial `LoginPage`.
...
- Support dark mode.
- Tests E2E Playwright (web) + Detox (mobile) sur golden paths.

### Out

- Refonte écran inscription / création cabinet (feature distincte).
- SSO SAML enterprise (réservé aux cabinets ≥20 IDEL, phase ultérieure).
...
- Changer le backend d'authentification (Struts legacy reste la source de vérité jusqu'à la migration API v2).
...
- **Backend** : endpoints `/api/v2/auth/login`, `/api/v2/auth/reset-password` déjà disponibles (emotion-back).
- **Infra** : client OAuth configuré côté DevOps (agatheyou repo).
