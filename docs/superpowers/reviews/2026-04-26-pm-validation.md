...
| 2 | Layout single-repo: `agents/<name>/agent.md` + `workflows/<name>/workflow.json` | DELIVERED | E2E test seeds exactly this layout: `cba-feature-dev-techdesign.e2e.test.ts:71-73`. P0 helper produces it: `git-resource-path.ts:34` |
...
| 8 | Repo target: `lab.cbainfo.fr/tom.andrieu/mnm-demo` | OPS GAP | Plan §M2 SQL exists but not committed as a script file. Repo restructure script also not committed. |
...
|---|---|---|---|
| 1 | Path convention `agents/<name>/agent.md`, `workflows/<name>/workflow.json`, gates | DELIVERED | E2E test seeds + asserts the convention end-to-end (`cba-feature-dev-techdesign.e2e.test.ts:70-74`). Gate path translation auto-derived via `workflowDir` from `governed-workflows-source-resolver.ts` (per plan §2.2). |
...
| 2 | P11 E2E test returns triplet `(agent_name, subagent_type, prompt_context)` without errors | DELIVERED | `cba-feature-dev-techdesign.e2e.test.ts:224-230` asserts `agentName: "senior-dev", subagentType: "mnm--senior-dev", promptContext: { ticket_id: "AY-10074" }`. Test runs full stack: setupWorkspace → launchWorkflow → launchStep with discovered sha. |
...
- [ ] M1 bash script not committed — Tom must reconstruct from plan §M1 prose and verify on lab.cbainfo.fr.
...
Per orchestration log line 38, isolated-vm DLL crash blocks local test runs on Windows. Tom MUST run `bun run test:e2e` (or full `bun test`) on Linux CI before demo to confirm zero regressions on the 3 adapted userId tests (`governed-workflows.test.ts:202,354,803`) and the new P11 E2E test (`cba-feature-dev-techdesign.e2e.test.ts`).
