# Workflow démo your organization — `feature-dev`

*Spec — 2026-04-25 — founder × Claude*

## 1. Contexte et intention

Démo MnM devant l'audience plénière your organization (CEO, CTO, PM/PO, lead tech, lead dev, dev,
...
diffusable à tout your organization via les settings Anthropic Console — sans construire ni
...
| Tous | Conclusion : 1 fichier YAML + 2 gates, diffusable à tout your organization, zéro UI à maintenir. |
...
Tous les artefacts sont à créer dans **gitlab.example.com/your-username/** (perso the maintainer).
...
| Reviewers de la MR | 2 comptes gitlab.example.com (un sera the maintainer, un sera un compte secondaire ou un collègue préparé) |
...
|---|---|---|
| 0:00–1:00 | Studio MnM, `workflow.json` | "Voici notre process feature your organization, codifié dans un fichier YAML. 4 steps. Imposé." |
| 1:00–1:45 | Studio, tab gates | "Deux gates : `approval-granted` et `mr-approved`. Du TypeScript lisible, pas du prompt." |
| 1:45–3:15 | Claude Code, `launch_governed_workflow(...)` | Step 1 démarre. `senior-dev` lit le ticket, écrit `design.md`, affiche le bloc d'approbation. the maintainer tape `APPROUVÉ — OK pour implémentation`. Gate verte. |
| 3:15–5:00 | Claude Code + onglet GitLab | Step 2 : agent code, écrit les tests, ouvre la MR. Le diff GitLab apparaît. |
| 5:00–6:30 | GitLab + Claude Code | Step 3 : MnM affiche "MR !X en attente de 2 approvals". the maintainer approuve depuis un autre onglet GitLab. Gate verte automatique. |
...
| 9:00–10:00 | Slide finale | "Ce que vous avez vu : 1 fichier YAML, 2 gates TS, 4 agents. Diffusable à tout your organization via les settings Anthropic Console. Zéro UI custom à maintenir." |
...
| L'audience ne comprend pas la conclusion "diffusable via Anthropic Console" | Slide finale explicite + une phrase de the maintainer : "vous installerez ce plugin demain matin sans rien faire". |
| Latence MCP atlassian côté your organization | Pré-charger un cache si possible, sinon afficher manuellement le ticket pendant que MnM tourne. |
...
- **Diffusion à your organization via Anthropic Console** — packaging du plugin MnM avec ce
  workflow comme exemple template.
- **Vrai workflow your organization AY** — adaptation de `feature-dev` aux conventions
  your organization réelles (branches `feat/AY-XXXX`, conventional commits, projet GitLab AY,
  Jira AY).
