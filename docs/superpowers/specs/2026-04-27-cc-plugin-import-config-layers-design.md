...
**Date** : 2026-04-27
**Auteur** : the maintainer + Claude
**Cas d'usage validateur** : `https://gitlab.example.com/example-org/hub/creation/lint-pack`
**Statut** : à valider par the maintainer avant écriture du plan d'implémentation.
...
**Gap** : Le hub interne your organization `https://gitlab.example.com/example-org/hub/` contient déjà N plugins Claude Code prêts (skills + agents + parfois MCP/hooks) — `lint-pack`, `quality-pack`, etc. MnM n'a aujourd'hui **aucun moyen** :
...
**Modèle visé (the maintainer)** : un `config_layer` MnM **est** l'équivalent d'un plugin CC. Le format on-disk vit à côté de `agents/` et `workflows/` dans le repo Git per-company. À terme, chaque `config_layer` peut être extrait dans son propre repo distant (modèle hub-style), mais on commence simple : tout dans le repo company.
...
| D5 | **Workflow `lint-pack` écrit à la main** dans `workflows/`, pas auto-généré depuis le skill orchestrateur. Les skills orchestrateurs (`test`) sont **exclus à l'import** via paramètre `exclude_skills`. | Trop magique pour V1, et l'intérêt MnM = ajouter des gates par-dessus. |
...
Après `import_cc_plugin(url="…/lint-pack", exclude_skills=["test"])` :
...
workflows/
  lint-pack.workflow.json   ← écrit à la main
  gates/
    lint-pack/
      preflight.gate.ts
...
config_layers: [lint-pack]   ← AJOUTÉ À L'IMPORT
---
```
...
|---|---|---|---|
| `source_url` | text | oui | URL du repo CC plugin importé (ex: `https://gitlab.example.com/example-org/hub/creation/lint-pack`) |
...
- Cohérent avec la sémantique the maintainer : "config_layer = plugin CC".
...
```json
{
  ...
  "mnmCommitSha": "def456…",
  "tag": "plugin-imports/lint-pack/v1.0.0"
}
```
...
## 7. Workflow `lint-pack` + gates

### 7.1 `workflows/lint-pack.workflow.json`
...
|---|---|
| MCP du plugin | `lint-pack` n'en a pas. Item type `"mcp"` ouvert mais pas implémenté. |
...
3. **E2E** : import réel `lint-pack` sur company de test, lancer le workflow `lint-pack` sur un projet Symfony fixture (Docker compose), gates passent.
...
- Format final du frontmatter `config_layers:` : liste de strings `["lint-pack"]` ou objets `[{name: ..., version: ...}]` ? V1 = strings (plus simple), version trackée séparément en DB.
