...
3. **`gitProvider`** (`packages/git-provider/`)
   ...
   - Tous les workflows ont déjà un repo Git source (`example-org/mnm-demo`). Le PAT compagnie permet d'écrire dedans.
...
2. Appelle `gitProvider.commitMultipleFiles({branch: "mnm-runs", actions: [...]})` sur le repo du workflow (`example-org/mnm-demo`) à un chemin `runs/<run_id>/<step_id>/<filename>`.
...
5. **Naturellement portable** : un dev pose `git clone example-org/mnm-demo && git checkout run/<run_id>/dev` → il a TOUT le contexte du step `dev` localement, prêt à reprendre le step `review` même hors-MnM.
...
### Phase 2 (post-pilote EnterpriseCustomer, quand le besoin émerge) : Option E hybride
...
3. **Encryption-at-rest sensitive content** : design.md peut contenir du contenu sensible (architecture interne). GitLab self-hosted EnterpriseCustomer = derrière VPN = OK pour le pilote. Pour SaaS futur, considérer git-crypt ou storage layer chiffré.
...
> Si MnM meurt après Phase 1, tu fais `git clone example-org/mnm-demo` et tous tes handoffs sont là, datés, signés, parcourables avec les outils Git natifs. C'est exactement le contrat que tu cherches.
