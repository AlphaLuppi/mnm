...
**Date** : 2026-04-27
**Origine** : the maintainer — "L'objectif grande finale c'est que CHAQUE étape puisse faire un /clear entre les étapes, et que ce qui prime ce soit le handoff entre les étapes. De sorte à ce que n'importe qui n'importe quand puisse reprendre la suite d'un workflow tant qu'il a les handoff précédent."
...
- L'utilisateur fait `/clear` entre deux steps (the maintainer le veut)
...
Pourquoi :
1. **Aligné avec la philosophie MnM** que the maintainer répète depuis le début : "if MnM dies the .md survive". Tout le monde peut cloner le repo et reprendre n'importe quel step en `git checkout <sha>`.
...
### Phase 2 (post-pilote your organization, quand le besoin émerge) : Option E hybride
...
3. **Encryption-at-rest sensitive content** : design.md peut contenir du contenu sensible (architecture interne). GitLab self-hosted your organization = derrière VPN = OK pour le pilote. Pour SaaS futur, considérer git-crypt ou storage layer chiffré.

4. **Permissions PAT** : le PAT compagnie doit avoir `write_repository`. the maintainer a déjà flag dans memory `project_pat-fallback-removal.md` que le PAT doit dégager post-démo au profit de l'OAuth user — quand ça arrivera, il faudra que l'identité OAuth user soit utilisée pour committer (ce qui est PLUS audit-friendly de toute façon : "the contributor a committé le handoff" vs "le PAT compagnie a committé").
