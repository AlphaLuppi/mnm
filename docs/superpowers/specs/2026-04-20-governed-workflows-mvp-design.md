...
```
User : "lance hello-world avec name=MnM founder"
├─ (1)  Harness → launchWorkflow("hello-world", {name:"MnM founder"})
...
├─ (5)  Serveur : pas d'entry gate, retourne {agent:"greeter", ctx:{name:"MnM founder"}}
├─ (6)  Harness → Task(subagent_type:"mnm--greeter", ...)
├─ (7)  Sub-agent produit {greeting:"Hello, MnM founder!"}
├─ (8)  Harness → completeStep(runId, "greet", {greeting:"Hello, MnM founder!"})
├─ (9)  Serveur : eval exit gate → pass
├─ (10) step greet=succeeded, unlock shout
├─ (11) Harness → launchStep(runId, "shout")
├─ (12) Serveur : retourne {agent:"shouter", ctx:{greeting:"Hello, MnM founder!"}}
...
| **T7** | ✅ shipped 2026-04-22 (`7c4a779..9a446e2`) | Polish + distribution : DEF-1 real mergeAgentConfig via `configLayerConflictService.mergePreview` (rename `env_ref`→`credential`), DEF-4 per-company `resolveGitProvider` (direct query sur `git_provider` config_layer_item, per-company cache, fail-closed `GIT_PROVIDER_MISCONFIG`), DEF-9 multi-company OAuth enforcement (explicit `company_id` required pour users avec ≥2 memberships, `MultiCompanyError` + `sendOAuthCompanyError` helper, SPA threading via `OAuthConsent.tsx`), T6 hot-reload spike verdict appliqué au plugin README + aux hints `AGENTS_STALE` (`/reload-plugins` obligatoire), onboarding skill `mnm--onboard`, spec manifest marketplace. | 60+ tests verts (resolve-git-provider: 10, mcp-oauth-multi-company: 4, governed-workflows partition: 1, + régressions). Happy path hello-world désormais exécutable via `/plugin install mnm@mnm-platform` → `/mnm--onboard`. | See `docs/superpowers/plans/2026-04-22-T7-polish-distribution.md` completion report |
