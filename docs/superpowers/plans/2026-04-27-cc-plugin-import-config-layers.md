...
**Goal:** Implémenter le pipeline d'import d'un plugin Claude Code (clone GitLab → split en agents standalone + 1 config_layer skills) puis livrer le workflow `symfony-upgrade-tests` avec ses 5 gates artifact-based, prêt pour la démo vendredi.
...
curl -X POST http://localhost:3100/api/companies/$COMPANY_ID/governed-workflows/import-plugin \
  ...
  -d '{"repo_url":"https://lab.cbainfo.fr/genia/hub/creation/symfony-upgrade-tests","exclude_skills":["test"]}'
```
...
## Task 11: Workflow `symfony-upgrade-tests.workflow.json`

**Files:**
- Create: `<company-repo>/workflows/symfony-upgrade-tests.workflow.json` (commité via le repo company, pas dans le repo MnM)
...
Identifier le repo company (cf `~/.mnm/dev-workflows-bare/repo.git` ou GitLab `tom.andrieu/mnm-workflows-tom`). Cloner localement pour édition.
...
```bash
cd <company-repo>
git add workflows/symfony-upgrade-tests.workflow.json
git commit -m "feat(workflow): add symfony-upgrade-tests"
git push
```
...
**Files:**
- Create: `<company-repo>/workflows/gates/symfony-upgrade-tests/phpstan-level-10.gate.ts`
- Create: `<company-repo>/workflows/gates/symfony-upgrade-tests/__tests__/phpstan-level-10.gate.test.ts` (si tests gates supportés dans le repo company ; sinon tests dans `packages/gate-runner/canonical/__tests__` si on les fait canoniques — V1 démo : repo company sans tests, ou copy-paste les fixtures pour smoke).
...
```bash
cd <company-repo>
git add workflows/gates/symfony-upgrade-tests/phpstan-level-10.gate.ts
...
**Files:**
- Create: `<company-repo>/workflows/gates/symfony-upgrade-tests/phpunit-pass.gate.ts`
...
**Files:**
- Create: `<company-repo>/workflows/gates/symfony-upgrade-tests/infection-msi.gate.ts`
...
**Files:**
- Create: `<company-repo>/workflows/gates/symfony-upgrade-tests/reviewer-approves.gate.ts`
...
**Files:**
- Create: `<company-repo>/workflows/gates/symfony-upgrade-tests/preflight.gate.ts`
...
**But :** valider end-to-end avec le vrai plugin `symfony-upgrade-tests` et un projet Symfony cible.
...
S'assurer que la company a un PAT GitLab CBA configuré (cf `docs/governed-workflows-gitlab-setup.md`).
...
Cloner / pull le repo company, vérifier la présence de `agents/test-writer.md`, `agents/test-reviewer.md`, `config_layers/symfony-upgrade-tests/`.

- [ ] **Step 5: Push le workflow + gates dans le repo company**

(Si pas déjà fait via Tasks 11-16.) Push les fichiers `workflows/symfony-upgrade-tests.workflow.json` + 5 gates.

- [ ] **Step 6: Lancer le workflow**

```bash
curl -X POST "http://localhost:3100/api/companies/$COMPANY_ID/governed-workflows/symfony-upgrade-tests/runs" \
  -H "Content-Type: application/json" \
  -d '{
    "variables": {
...
Ouvrir `/workflows/symfony-upgrade-tests/runs/<run-id>`, vérifier que les 3 steps se déroulent et que les 5 gates passent (ou expliquent clairement quoi est manquant).

- [ ] **Step 8: Tag final pour la démo**

```bash
git tag -a demo-2026-04-30-cc-plugin-import -m "Demo Friday: CC plugin import + symfony-upgrade-tests workflow"
git push --tags
```
