    ...
    const result = await runSingleGate(
      {
        ...
        context: ctx({
          step: { id: "shout", previous_artifacts: { greet: { greeting: "Hello, MnM founder!" } } },
        }),
      },
      { compiledCache: new CompiledCache() },
    );
    expect(result.pass).toBe(true);
    expect(result.report).toBe("previous greeting: Hello, MnM founder!");
  });
...
function baseCtx(overrides: Partial<GateContext> = {}): GateContext {
  return {
    artifact: undefined,
    run: { id: "run-1", workflow_name: "hello-world", git_tag: "v1.0.0", params: { name: "MnM founder" } },
    step: { id: "greet", previous_artifacts: {} },
    config: {},
    kind: "exit",
    helpers: {},
    ...
    const result = await runSingleGate(
      {
        ...
        kind: "exit",
        context: baseCtx({
          artifact: { greeting: "Hello, MnM founder!" },
