...
> **But** : finir le strict nécessaire pour publier proprement avant que Tom forke chez CBA pour usage interne.
...
Audit initial 2026-04-28 : **170 occurrences résiduelles** dans 30 fichiers trackés (CBA, cbainfo, lab.cba, paperclip, emails). À nettoyer.
...
**Règles** :
- Retirer toute mention `CBA`, `cbainfo`, `lab.cba`, `lab.cbainfo.fr`, emails internes en clair (`@cbainfo.fr`, etc.), noms de personnes physiques côté client.
...
- Remplacer `lab.cbainfo.fr` → `https://example.com` ou supprimer la ligne si c'est un commentaire.
...
**AC** : `git grep -i -E "CBA|cbainfo|lab\.cba" -- ':!docs/superpowers/specs/T7-marketplace-manifest.md' ':!bun.lock' ':!_bmad/_config/files-manifest.csv'` retourne 0 hits suspects. Faux positifs (CBA dans un nom de constante neutre type `RBAC`, `cbainfo` dans une URL externe légitime) à documenter.

**Output** : 1 commit `chore(open-source): finish Phase 0 — scrub residual CBA/cbainfo/internal refs`.
...
- `lab.cbainfo.fr` → `example.com`
- `cbainfo.fr` → `example.com`
- emails `*@cbainfo.fr` → `internal@example.com`
...
4. Vérifier `git log -p | grep -i cbainfo` retourne rien.
...
**AC** : `git log --all -p | grep -i -E "cbainfo|lab\.cba"` retourne 0. Branche pushable en force-with-lease.
