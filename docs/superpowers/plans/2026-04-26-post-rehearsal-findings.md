...
Le subagent review-watcher a refusé de fabriquer `approvals_count: 1` à partir de `approved: true` malgré la pression du contexte ("the maintainer a déjà approuvé"). Il a bloqué tant que `approved_by[]` était vide. the maintainer a dû effectivement cliquer le bouton Approve pour débloquer.
