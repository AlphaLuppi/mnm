# Brainstorm — Use case your organization : Refonte enterprise product
...
- **Client :** your organization (santé, réglementaire fort)
...
|------|--------|----------|
| **Archi Angular** | Refonte from scratch, sait dev back backend-framework, utilise IA au quotidien. A poussé plugin `team-dev-frontend` (plusieurs skills) | Crée workflows + agents, supervise |
| **Dev Backend** | Java/backend-framework, sait faire du front, a poussé plugin `team-dev-backend` | Exécute workflows dev |
...
### Phase 5 — Dev front
- **Step `dev-front-angular`** (agent `dev-frontend-team`, skills `team-dev-frontend`)
...
4. **Infra GitProvider** — Cloner/pull/push repos workflow. Interface + impl GitLab minimum (the maintainer parle de GitLab en entreprise ? ou GitHub ?).
...
9. **Plugin skills invocation** — Agent `dev-frontend-team` utilise plugin `team-dev-frontend`. MnM doit pouvoir invoquer un agent Claude Code avec skills préchargées. (Via `claude -p` + `--skills` ?)
...
### Manques côté client your organization
...
18. **Plugins your organization packaging** — `team-dev-frontend` et `team-dev-backend` sont des plugins Claude Code. Doivent être versionnés, distribués à l'agent qui tourne pour MnM.

## Questions à trancher avec the maintainer
...
## Décisions de the maintainer (Round 1)
...
## Questions the maintainer (à clarifier)
...
- **Questions** :
  - Quand Claude Code local lance un step qui est un agent (ex: `dev-frontend-team`), cet agent tourne en local aussi ? Ou c'est un sous-process Claude Code qui ré-invoque `claude -p --plugin team-dev-frontend` ?
...
- Principe the maintainer : **tout au format portable**.
...
- Les hooks vivent dans des **plugins Claude Code pré-installés company-wide** (ex: plugin `team-dev-workflows` installé sur tous les postes your organization).
...
**Protocole** :
1. MnM dit à Claude Code : "exécute step X avec agent `dev-frontend-team@2.3.1`".
2. Claude Code vérifie cache local : ai-je `dev-frontend-team@2.3.1` ?
   - **Oui** → utilise direct.
   - **Non** → télécharge depuis MnM : `GET /companies/:companyId/agents/dev-frontend-team/2.3.1/bundle`.
...
- Proposition :
  ```
  agents/dev-frontend-team/              ← 1 repo git
...
```yaml
# agents/dev-frontend-team/agent.yml
name: dev-frontend-team
version: 2.3.1
adapter_type: claude_local
description: "Dev front Angular spécialisé your organization enterprise product"
...
## Recap complet du design your organization
...
24. `dev-frontend-team` (existe via plugin team-dev-frontend)
...
## Round 6 — Roadmap POC your organization (à venir)
...
- Qui code quoi (the maintainer + the lead developer dispos ?) ?
- Priorité : démo fonctionnelle vs robustesse ?

## Prochains rounds

- Round 5 : format repo agent + config layer + routine nightly sync
- Round 6 : roadmap POC minimal your organization (1 feature pilote enterprise product)
