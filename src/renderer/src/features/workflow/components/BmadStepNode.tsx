...
export function BmadStepNode({ data, selected }: { data: StepNodeData; selected?: boolean }) {
  ...
  return (
    <div
      className={`rounded-lg border px-4 py-3 min-w-[200px] bg-[var(--color-surface)] border-[var(--color-border)] text-[var(--color-text-primary)] ${selected ? 'border-[var(--color-accent)] ring-2 ring-[var(--color-accent)]/30' : ''} ${execClass} transition-all duration-200`}
      role="button"
      tabIndex={0}
